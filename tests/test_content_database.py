...
def test_clean_migration_and_ordering(tmp_path: Path) -> None:
    ...
    assert migrate(path) == 5
    assert migrate(path) == 0
    with connect(path) as db:
        versions = [r[0] for r in db.execute("SELECT version FROM schema_migrations ORDER BY version")]
    assert versions == sorted(versions) == ["001_content_database.sql", "002_ingestion_tracking.sql", "003_source_inventory.sql", "004_lick_corpus.sql", "005_phrase_corpus_relationship_graph.sql"]
