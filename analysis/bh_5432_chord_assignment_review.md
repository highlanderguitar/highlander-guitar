# BH-5432 chord-assignment review

All assignments are provisional and require listening review. Canonical C describes the instructional reference, not a permanent accompaniment chord.

| Section | Primary audible chord | Function | Alternatives | Confidence | Evidence/status |
|---|---|---|---|---:|---|
| from 5 | Cmaj7 | Imaj7 | C major, Cmaj9, C6, or G7 | 0.86 | source annotation; needs_review |
| from 4 | Cmaj9 | Imaj9 | Cmaj7, C major, or Fmaj7 | 0.72 | source annotation; needs_review |
| from 3 | Cmaj9 | Imaj9 | G13 or Cmaj7 | 0.58 | source annotation; needs_review |
| separator | none | none | unresolved | 1.00 | boundary inferred from source spacing/register; needs review; needs_review |
| from 2 | Cmaj9 | Imaj9 | G13 or Cmaj7 | 0.78 | source annotation; needs_review |
| ALL TOGETHER | Cmaj9 | Imaj9 | Cmaj7 or G13 | 0.67 | source annotation; needs_review |
| 9th arp | Cmaj9 | Imaj9 | Cmaj7(add9) or G13 | 0.84 | source annotation; needs_review |
| additional upper-position variants | Cmaj9 | Imaj9 | multiple plausible | 0.48 | boundary inferred from source spacing/register; needs review; needs_review |
| separator | none | none | unresolved | 1.00 | boundary inferred from source spacing/register; needs review; needs_review |
| low-position instructional material | none | none | Fmaj7, Gm/C, or C11; unresolved | 0.28 | boundary inferred from source spacing/register; needs review; needs_review |
| separator | none | none | unresolved | 1.00 | boundary inferred from source spacing/register; needs review; needs_review |
| upper-register application material | Cmaj9 | Imaj9 | G13 or Cmaj7 | 0.55 | boundary inferred from source spacing/register; needs review; needs_review |
| separator | none | none | unresolved | 1.00 | boundary inferred from source spacing/register; needs review; needs_review |
| closing transition variants | Cmaj9 | Imaj9 | G13 or Cmaj7 | 0.55 | boundary inferred from source spacing/register; needs review; needs_review |
